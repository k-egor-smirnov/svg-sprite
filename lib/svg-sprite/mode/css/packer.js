...
SVGSpriteCssPacker.prototype._splitNode = function(node, width, height, margin) {
		node.used					= true;
    node.down					= {x: node.x + margin.right, y: node.y + height + margin.top, width: node.width + margin.right, height: node.height - height + margin.top};
		node.right					= {x: node.x + width + margin.right, y: node.y, width: node.width - width + margin.right, height: height};
...
SVGSpriteCssPacker.prototype._growRight = function(width, height, margin) {
	this.root					= {
		...
		x						: 0,
		y						: 0,
		width					: this.root.width + width + margin.right + margin.left,
		height					: this.root.height + margin.top + margin.bottom,
		down					: this.root + margin.bottom,
...
SVGSpriteCssPacker.prototype._growBottom = function(width, height, margin) {
	this.root					= {
		...
		x						: 0,
		y						: 0,
		width					: this.root.width + margin.right + margin.left,
		height					: this.root.height + height + margin.top + margin.bottom,
		down					: {x: 0, y: this.root.height + margin.top, width: this.root.width + margin.right + margin.left, height: height + margin.top + margin.bottom},
		right					: this.root
    };
		var node					= this._findNode(this.root, width, height);
