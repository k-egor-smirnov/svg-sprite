...
SVGSpriteCss.prototype._layout = function() {
	...
	this.data.shapes.forEach(function(shape, index) {
		...
		if (this._displaceable || !shape.master) {
			...
			var padding = this._spriter.config.shape.spacing.padding;

			if (this._spriter.config.shape.spacing.box === 'padding') {
				shape.position.absolute.x -= padding.left;
				shape.position.absolute.y -= padding.top;
			}

			shape.position.absolute.xy = `${shape.position.absolute.x}px ${shape.position.absolute.y}px`;
			shape.width.outer = shape.width.inner;
			shape.height.outer = shape.height.inner;
			...
			if (svgX.length > 1) {
				x					= shape.master ? (shape.position.absolute.x - positionMap[shape.master].x) : shape.position.absolute.x + (this._spriter.config.shape.spacing.box === 'padding' ? padding.left : 0);
			...
			if (svgY.length > 1) {
				y					= shape.master ? (shape.position.absolute.y - positionMap[shape.master].y) : shape.position.absolute.y + (this._spriter.config.shape.spacing.box === 'padding' ? padding.top : 0);
